...
public:
    ...
    // Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, GCNextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    // void ExtractGCN(const cv::Mat &im, const int x0, const int x1);
    ...
    void Computex3DFromRGBD(const cv::Mat &imGray, const cv::Mat &imDepth);
...
public:
    ...
    std::vector<MapPoint*> mvpAllPoints;
    std::vector<Eigen::Vector3f> mpvx3D;
