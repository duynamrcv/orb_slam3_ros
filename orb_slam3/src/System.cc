    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    if(!node.empty() && node.isString() && node.string() == "1.0"){
        ...
        settings_ = nullptr;
        cv::FileNode node = fsSettings["System.AtlasFile"];
        if(!node.empty() && node.isString())
        {
            mStrLoadAtlasFromFile = (string)node;
        }

        node = fsSettings["System.AtlasFile"];
        if(!node.empty() && node.isString())
        {
...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    mMapPoints = mpAtlas->GetCurrentMap()->GetAllMapPoints();
    mKeyFrames = mpAtlas->GetCurrentMap()->GetAllKeyFrames();
...
//         if (localize_only) ActivateLocalizationMode();
//         else DeactivateLocalizationMode();
//     }
...
bool System::GetFindLocalization()
{
    return mpAtlas->GetFindLocalization();
}
