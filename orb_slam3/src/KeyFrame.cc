...
void KeyFrame::SetPose(const Sophus::SE3f &Tcw)
{
    ...
    // Create AllPoints associated to KeyFrame
    std::vector<Eigen::Vector3f> vx3D = Getx3D();
    for(unsigned int i = 0; i < vx3D.size(); i++)
    {
        vx3D[i] = mRwc*vx3D[i] + mTwc.translation();
        MapPoint* pNewMP = new MapPoint(vx3D[i], this, pMapKF);
        pMapKF->AddAllPoint(pNewMP);
        AddAllPoint(pNewMP,i);
    }
}
...
void KeyFrame::AddAllPoint(MapPoint *pMP, const size_t &idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvpAllPoints[idx]=pMP;
}
...
set<MapPoint*> KeyFrame::GetAllPoints()
{
    // std::cout << "KeyFrame: all points: " << mvpAllPoints.size() << std::endl;
    unique_lock<mutex> lock(mMutexFeatures);
    set<MapPoint*> s;
    for(size_t i=0, iend=mvpAllPoints.size(); i<iend; i++)
    {
        if(!mvpAllPoints[i])
            continue;
        MapPoint* pMP = mvpAllPoints[i];
        s.insert(pMP);
    }
    return s;
}

std::vector<Eigen::Vector3f> KeyFrame::Getx3D()
{
    unique_lock<mutex> lock(mMutexFeatures);
    return mvpx3D;
}
