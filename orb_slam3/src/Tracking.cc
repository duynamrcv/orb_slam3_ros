...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
        ...
        // mpIniGCNextractor = new GCNextractor(5*nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::RGBD)
        ...
        // mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpGCNextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
        // mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpGCNextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            ...
            // mCurrentFrame = Frame(mImGray,timestamp,mpIniGCNextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
        else
    ...
    else if(mSensor == System::IMU_MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        {
            ...
            // mCurrentFrame = Frame(mImGray,timestamp,mpIniGCNextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
        }
        else
