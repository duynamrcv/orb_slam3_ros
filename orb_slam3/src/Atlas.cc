...
Atlas::Atlas(int initKFid): mnLastInitKFidMap(initKFid), mHasViewer(false), mFindLocalization(false)
{
    mpCurrentMap = static_cast<Map*>(NULL);
    CreateNewMap();
}
...
void Atlas::AddAllPoint(MapPoint* pMP)
{
    Map* pMapMP = pMP->GetMap();
    pMapMP->AddAllPoint(pMP);
}
...
bool Atlas::GetFindLocalization()
{
    return mFindLocalization;
}

void Atlas::SetFindLocalization(bool value)
{
    mFindLocalization = value;
}
