...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     mbf(frame.mbf), mb(frame.mb), mThDepth(frame.mThDepth), N(frame.N), Nall(frame.Nall), mvKeys(frame.mvKeys),
     mvKeysRight(frame.mvKeysRight), mvKeysUn(frame.mvKeysUn), mvuRight(frame.mvuRight),
     mvDepth(frame.mvDepth), mBowVec(frame.mBowVec), mFeatVec(frame.mFeatVec),
     mpvx3D(frame.mpvx3D), mDescriptors(frame.mDescriptors.clone()), mDescriptorsRight(frame.mDescriptorsRight.clone()),
     mvpMapPoints(frame.mvpMapPoints), mvpAllPoints(frame.mvpAllPoints), mvbOutlier(frame.mvbOutlier), mImuCalib(frame.mImuCalib), mnCloseMPs(frame.mnCloseMPs),
     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    mvpAllPoints = vector<MapPoint*>(Nall,static_cast<MapPoint*>(NULL));
     ...
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    mvpMapPoints = vector<MapPoint*>(N,static_cast<MapPoint*>(NULL));
    mvpAllPoints = vector<MapPoint*>(Nall,static_cast<MapPoint*>(NULL));
    mpvx3D = vector<Eigen::Vector3f>(Nall,static_cast<Eigen::Vector3f>(NULL));
    ...
    Computex3DFromRGBD(imGray, imDepth);
...
// void Frame::ExtractGCN(const cv::Mat &im, const int x0, const int x1)
// {
//     vector<int> vLapping = {x0,x1};
//     monoLeft = (*mpGCNextractor)(im,cv::Mat(),mvKeys,mDescriptors,vLapping);
// }
...
void Frame::Computex3DFromRGBD(const cv::Mat& imGray, const cv::Mat& imDepth)
{
    int v = int(imGray.rows/2);
    for(int u = 0; u < imGray.cols; u+=step)
    {
